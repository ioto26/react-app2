...
// --- TypeScriptのためのグローバルTHREE変数の宣言 ---
// この行は、TypeScriptに 'THREE' という変数がグローバルスコープに存在することを伝えます。
// これにより、importせずにTHREE.Meshなどの型を使用できます。
declare const THREE: any;


// --- 3Dオブジェクト生成ヘルパー関数 ---
// ロジックは変更ありませんが、グローバルなTHREEオブジェクトに依存します。
 ...
 */
const createBook = (color: any, size: [number, number, number]): any /* THREE.Mesh */ => {
 ...
 */
const createShelfMesh = (args: [number, number, number]): any /* THREE.Mesh */ => {
 ...
 */
const createBookshelf = (width: number, height: number, depth: number, thickness: number): any /* THREE.Group */ => {
 ...
 * キラキラ光るパーティクルを作成します。
 */
const createSparkles = (): any /* THREE.Points */ => {
    const vertices = [];
    for (let i = 0; i < 100; i++) {
...
export default function App() {
  ...
  useEffect(() => {
    // --- CDNから読み込んだTHREEライブラリの存在を確認 ---
    if (typeof THREE === 'undefined' || !THREE) {
        console.error("Three.js has not been loaded. Please check the script tags in your index.html.");
        // UIにエラーメッセージを表示することもできます
        if(mountRef.current) {
            mountRef.current.innerHTML = `<div style="color: red; padding: 20px;">エラー: Three.jsが読み込めませんでした。index.htmlのscriptタグを確認してください。</div>`;
        }
        return;
    }

    // CDNスクリプトからロードされたクラスを取得
    ...
    // mountRef.currentがnullでないことを確認してから子要素を追加
